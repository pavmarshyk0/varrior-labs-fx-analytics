...
> **Repository sync notice (20 August 2026):** M3B and Dashboard V2.1 are verified in the local workstation copy, but their source files are not yet synchronized to GitHub `main`. Gen-3 events, matched controls and outcomes have not been run. See [`docs/PROJECT_STATUS.md`](docs/PROJECT_STATUS.md) for the exact local/public split.
...
Latest verified local checkpoint: **111 passed, 2 skipped, 13 subtests passed**. This result belongs to the newer local M3B/Dashboard V2.1 tree and is not yet reproducible from public `main` until that exact source is synchronized.
